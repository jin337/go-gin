...
func UpdateUser(ctx *gin.Context, DB *gorm.DB) (interface{}, error) {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		// 提交更新
		err := tx.Model(&item).Updates(updateData).Error
		if err != nil {
			return err
		}
		...
		// 更新内容
		if err := tx.Model(&model.Account{}).Where("user_id=?", item.ID).Updates(accountData).Error; err != nil {
			return err
		}

		// 查询更新数据
		if err := tx.Preload("Account").First(&item, req.ID).Error; err != nil {
			return err
		}
		return nil
	})
...
func DeleteUser(ctx *gin.Context, DB *gorm.DB) (interface{}, error) {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		if item.Account.ID != 0 {
			// 更新is_active为0
			if err := tx.Model(&item.Account).Update("is_active", 0).Error; err != nil {
				return err
			}

			// 删除账户记录
			if err := tx.Delete(&item.Account).Error; err != nil {
				return err
			}
		}
